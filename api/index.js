...
// Explicitly set views directory (important for Vercel serverless environment)
app.set('views', path.join(__dirname, '..', 'views'));
app.set('view engine', 'ejs');

// Multer setup for file uploads (memory storage → GridFS)
...
app.use(session({
  ...
  cookie: {
    ...
    secure: false // Temporary for debugging – change to true in production
  }
}));
...
// Initialize DB connection once (runs when function cold-starts)
(async () => {
  await connectDB();
})().catch(console.error);

// ────────────────────────────────────────────────
// All routes below (copied exactly from your original)
// ────────────────────────────────────────────────
